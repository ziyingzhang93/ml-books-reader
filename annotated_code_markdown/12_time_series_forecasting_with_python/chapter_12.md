# 时间序列预测 / Time Series Forecasting with Python
## Chapter 12

---

### Chapter Summary / 章节总结

# Chapter 12 Summary / 第12章总结

## Theme / 主题: Chapter 12 / Chapter 12

This chapter contains **3 code files** demonstrating chapter 12.

本章包含 **3 个代码文件**，演示Chapter 12。

---
## Evolution / 演化路线

  1. `airline_multiplicative.ipynb` — Airline Multiplicative
  2. `contrived_additive.ipynb` — Contrived Additive
  3. `contrived_multiplicative.ipynb` — Contrived Multiplicative

---
## ML Relevance / ML 关联

The techniques in this chapter (Chapter 12) are fundamental building blocks in machine learning pipelines.

本章技术（Chapter 12）是机器学习流水线中的基础构建块。

---

### Airline Multiplicative



---

### Contrived Additive



---

### Contrived Multiplicative



---
